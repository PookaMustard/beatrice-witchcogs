...
try:
    from py_bing_search import PyBingImageSearch, PyBingWebSearch, \
        PyBingVideoSearch, PyBingNewsSearch
    bingavailable = True
except:
    bingavailable = False
...
class Bing:
    ...
    def getfrombing(self, apikey, text, limit, operation):
        if operation == 'moderateimagesearch':
            bing_obj = PyBingImageSearch(
                apikey, text, custom_params="&Adult='Moderate'")
        elif operation == 'strictimagesearch':
            bing_obj = PyBingImageSearch(
                apikey, text, custom_params="&Adult='Strict'")
        elif operation == 'adultimagesearch':
            bing_obj = PyBingImageSearch(
                apikey, text, custom_params="&Adult='Off'")
    ...
    def obtainresult(self, result, operation):
        maxnum = len(result)
        if operation == 'moderateimagesearch' or \
            operation == 'videosearch' or \
                operation == 'strictimagesearch' or \
    ...
    def checkadult(self, server, channel, settings):
        ...
        if adultchannel == 'False':
            return False
        elif (adultchannel == 'True' or adultchannel == 'Unknown') \
                and adultserver == 'True':
            return True
    ...
    async def bingsetadult(self, ctx, setting):
        ...
        message = ctx.message
        if setting == 'channel':
            await self.bot.say("Do you want to enable %bingadult for this " +
                               "channel? This will enable this " +
                               "channel to use the %bingadult command, " +
                               "which image searches Bing with Safe Search " +
                               "turned off. Do note that this setting will " +
                               "override the global server setting and thus " +
                               "will allow %bingadult in this channel even " +
                               "if the global server setting is off.\n**ARE " +
                               "YOU SURE YOU WANT TO TOGGLE %bingadult?**" +
                               "\n(y/n)")
        elif setting == 'server':
            await self.bot.say("Do you want to enable %bingadult for this " +
                               "server? This will enable your server to use " +
                               "the %bingadult command, which image " +
                               "searches Bing with Safe Search turned off. " +
                               "Do note that this setting will be overriden " +
                               "per channel if a channel is set to accept " +
                               "usage of %bingadult. **ARE YOU SURE YOU " +
                               "WANT TO TOGGLE %bingadult?**\n(y/n)")
        else:
            return await self.bot.say("This command accepts either server " +
                                      "or channel. Please use it again.")
        response = await self.bot.wait_for_message(author=message.author)
        if response.content.lower().strip() == "y":
            if setting == 'channel':
                self.setadultchannel(channel, 'True')
                return await self.bot.say("Enabled %bingadult settings for " +
                                          "this channel.")
            elif setting == 'server':
            ...
            if setting == 'channel':
                ...
                return await self.bot.say("Disabled %bingadult settings for " +
                                          "this channel.")
            elif setting == 'server':
...
def setup(bot):
    if bingavailable:
        ...
        raise RuntimeError("You need to run 'pip3 install py-bing-search', " +
